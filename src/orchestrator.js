...
async function publishBlocked({ store, publisher, config, snapshot, reason, job, now }) {
  ...
  const result = await publisher.publish(report, snapshot, { now });
  store.completeJob(job.id, now);
  return result;
}

export async function processJob({ job, store, config, adapter, luna, publisher, fixture = null, now = new Date().toISOString() } = {}) {
  let snapshot;
  try {
    snapshot = fixture ? await loadFixture(fixture, { normalize: normalizeLeagueResponse }) : await adapter.readSnapshot();
    store.saveSnapshot(snapshot);
  } catch (error) {
    store.failJob(job.id, error.message, { retry: false });
    return { status: "failed", error: error.message };
  }
  if (snapshot.criticalMissing.length) return publishBlocked({ store, publisher, config, snapshot, reason: `critical ESPN fields are missing: ${snapshot.criticalMissing.join(", ")}`, job, now });
  if (!luna) return publishBlocked({ store, publisher, config, snapshot, reason: "OpenAI client is not configured for this worker", job, now });

  const previous = store.latestReport(reportIdentity(snapshot));
  const attempts = 1 + Number(config.orchestration.bounded_retries || 0);
  let specialist;
  try {
    specialist = await retry("specialist research", attempts, () => luna.call({ input: buildSpecialistInput({ config, job, snapshot, previousReports: previous ? [previous] : [] }), purpose: `specialist:${job.job_type}`, webSearch: true }));
  } catch (error) {
    return publishBlocked({ store, publisher, config, snapshot, reason: error.message, job, now });
  }
  ...
  try {
    editor = await retry("research editor", attempts, async () => {
      const response = await luna.call({ input: buildEditorInput({ config, job, snapshot, specialistResults, previousReports: previous ? [previous] : [] }), purpose: "research_editor", structuredSchema: editorSchema.$defs.ReportPayload, webSearch: true });
  ...
  if (!validation.valid) return publishBlocked({ store, publisher, config, snapshot, reason: `editor report failed deterministic validation: ${validation.errors.join("; ")}`, job, now });
  const result = await publisher.publish(report, snapshot, { now });
  store.completeJob(job.id, now);
  return result;
}
...
export async function workerOnce({ store, config, adapter, luna, publisher, fixture = null, workerId = `worker-${process.pid}`, now = new Date().toISOString() } = {}) {
  ...
  try { return await processJob({ job, store, config, adapter, luna, publisher, fixture, now }); } catch (error) {
    store.failJob(job.id, error.message, { retry: job.attempts < 1 + Number(config.orchestration.bounded_retries || 0) });
    return { status: "failed", error: error.message };
  }
}
